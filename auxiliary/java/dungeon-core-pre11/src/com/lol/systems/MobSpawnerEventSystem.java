...
import com.lol.events.SignalEvent;
import com.lol.utils.BlockHelper;
import org.checkerframework.checker.nullness.compatqual.NonNullDecl;
import org.checkerframework.checker.nullness.compatqual.NullableDecl;
import org.jline.utils.Log;

public class MobSpawnerEventSystem
extends EntityEventSystem<ChunkStore, SignalEvent> {
    public MobSpawnerEventSystem() {
        super(SignalEvent.class);
    }

    public Holder<EntityStore> makeEntity(String role) {
    ...
    public void handle(int i, @NonNullDecl ArchetypeChunk<ChunkStore> archetypeChunk, @NonNullDecl Store<ChunkStore> store, @NonNullDecl CommandBuffer<ChunkStore> commandBuffer, @NonNullDecl SignalEvent signalEvent) {
        if (!signalEvent.signal) {
            return;
        }
        Log.info((Object[])new Object[]{"Spawning"});
        Vector3i pos = BlockHelper.getPosForBlock((Ref<ChunkStore>)archetypeChunk.getReferenceTo(i), commandBuffer);
        if (pos == null) {
            return;
        }
        ++pos.y;
        World w = ((ChunkStore)commandBuffer.getExternalData()).getWorld();
        MobSpawner ms = (MobSpawner)archetypeChunk.getComponent(i, MobSpawner.getComponentType());
        if (ms == null) {
            return;
        }
        for (int j = 0; j < ms.count; ++j) {
            ...
            w.execute(() -> {
                Ref<EntityStore> mob = this.spawn(spawnPos, w, this.makeEntity(ms.role));
    ...
    public Query<ChunkStore> getQuery() {
        return MobSpawner.getComponentType();
    }
}
