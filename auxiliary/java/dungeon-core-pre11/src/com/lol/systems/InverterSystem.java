...
extends RefSystem<ChunkStore> {
    ...
    public Query<ChunkStore> getQuery() {
        return Query.and((Query[])new Query[]{Inverter.getComponentType(), SignalSender.getComponentType()});
    }
    ...
    public void onEntityAdded(@NonNullDecl Ref<ChunkStore> ref, @NonNullDecl AddReason addReason, @NonNullDecl Store<ChunkStore> store, @NonNullDecl CommandBuffer<ChunkStore> commandBuffer) {
        ...
        router.registerListener(new SignalRouter.ListenerRegister(rec.getId(), SignalRouter.NOR, bools -> {
            boolean active = bools[0];
            if (!sender.active) {
                return;
            }
            router.sendSignal(new SignalRouter.LogicSignal(sender.id, active));
        }));
    }
