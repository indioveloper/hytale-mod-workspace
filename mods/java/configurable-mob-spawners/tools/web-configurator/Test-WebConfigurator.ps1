...
foreach ($required in @("mobPreview", "mobName", "playerReference", "equipmentDock", "Modelo NPC", "roleOptions", "roleToggle", "roleModal", "roleCategories", "mob-preview.generated.js", "updatePreviewGeometry", "previewTitle", "previewDescriptor")) {
...
foreach ($removed in @('playerReferenceId', 'loadPlayerReference', 'player-preview.generated.js', 'raynor')) {
...
foreach ($required in @('itemModal', 'itemSearch', 'itemCategories', 'HYTALE_ITEM_GROUPS', 'data-item-filter="hand"', 'data-item-filter="block"', 'data-item-filter="head"', 'Escudos', 'Armaduras', 'Tejados', 'Bloque en la mano', 'Rango de spawn', 'syncSpawnCounts', 'resetSpawnSimulation', 'animateSpawnerMap', 'activationLegend', 'lightLegend', 'spawnLegend', 'describeLoot', 'addTag', 'tagList', 'lootEditor', 'addLoot', 'clearItemSearch')) {
  if (-not $html.Contains($required)) { throw "Filtered item picker hook is missing: $required" }
}
foreach ($required in @('Mob Spawner Customizable', 'helpOpen', 'helpModal', 'helpClose', '1) Mob y Oleadas', '2) Spawner', '3) Armas y armadura', '4) Loot', '>Ejemplo<', 'Exportar Spawner', 'export-string', 'updateExport', 'Importar spawner', 'syncLootRow', 'syncAllLoot')) {
...
foreach ($required in @('window.HYTALE_ITEM_CATALOG=', 'window.HYTALE_ITEM_GROUPS=', 'window.HYTALE_ITEM_PREVIEWS=')) {
...
$groupMatch = [regex]::Match($itemRaw, 'window\.HYTALE_ITEM_GROUPS=(\{.*?\});window\.HYTALE_ITEM_PREVIEWS=')
