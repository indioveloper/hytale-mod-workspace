...
& $buildScript -ProjectPath (Join-Path $repo "mods\java\more-triggers") -SourceRoot src -PackageRoot src -ArtifactName More_Triggers-1_10_1.jar -ServerJar $ServerJar
Invoke-ProjectScript "mods\java\more-triggers\tools\Test-Package.ps1" @{ ArchivePath = (Join-Path $repo "mods\java\more-triggers\.build\dist\More_Triggers-1_10_1.jar") }
